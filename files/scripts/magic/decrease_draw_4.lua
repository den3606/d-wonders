...
local function decrease_draw(wand_entity_id, decrease_drow_size, is_enabled)
  ...
  local decrease_draw_count_array_serialized = GetInternalVariableValue(wand_entity_id, 'decrease_draw_4.decrease_draw_count_array', 'value_string')
  if decrease_draw_count_array_serialized == nil then
    decrease_draw_count_array = {}
    AddNewInternalVariable(wand_entity_id, 'decrease_draw_4.decrease_draw_count_array', 'value_string', Json.encode(decrease_draw_count_array))
  else
  ...
  SetInternalVariableValue(wand_entity_id, 'decrease_draw_4.decrease_draw_count_array', 'value_string', Json.encode(decrease_draw_count_array))
end
...
function enabled_changed(spell_entity_id, is_enabled)
  ...
  if is_enabled then
    if string.find(tags, "wand") then
      local wand_entity_id = GetInternalVariableValue(spell_entity_id, 'decrease_draw_4.decrease_draw_target_wand_entity_id', 'value_int')
      if wand_entity_id == nil then
        AddNewInternalVariable(spell_entity_id, 'decrease_draw_4.decrease_draw_target_wand_entity_id', 'value_int', parent_entity_id)
      end
    end
  else
    local wand_entity_id = GetInternalVariableValue(spell_entity_id, 'decrease_draw_4.decrease_draw_target_wand_entity_id', 'value_int')
    ...
    RemoveInternalVariableValue(spell_entity_id, 'decrease_draw_4.decrease_draw_target_wand_entity_id')
  end

  decrease_draw(parent_entity_id, 4, is_enabled)
end
