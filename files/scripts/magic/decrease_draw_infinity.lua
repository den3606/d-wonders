...
local function add_spell_in_wand_detection(spell_entity_id)
  local watch_wand_component_id = GetInternalVariableValue(spell_entity_id, DECREASE_DRAW.VARIABLE_KEYS.START_WATCH_WAND, "value_int")
  if watch_wand_component_id == nil then
    watch_wand_component_id = EntityAddComponent2(spell_entity_id, "LuaComponent", {
      ...
      script_source_file = "mods/d-wonders/files/scripts/magic/decrease_draw_infinity_add_detection.lua",
      execute_every_n_frame = 1,
    })
    AddNewInternalVariable(spell_entity_id, DECREASE_DRAW.VARIABLE_KEYS.START_WATCH_WAND, "value_int", watch_wand_component_id)
  end
end
...
local function remove_spell_in_wand_detection(spell_entity_id)
  ...
  if watch_wand_component_id then
    RemoveInternalVariableValue(spell_entity_id, DECREASE_DRAW.VARIABLE_KEYS.START_WATCH_WAND)
    EntityRemoveComponent(spell_entity_id, watch_wand_component_id)
  end
end

local function add_spell_out_wand_detection(spell_entity_id)
  local watch_wand_component_id = GetInternalVariableValue(spell_entity_id, DECREASE_DRAW.VARIABLE_KEYS.END_WATCH_WAND, "value_int")
  if watch_wand_component_id == nil then
    watch_wand_component_id = EntityAddComponent2(spell_entity_id, "LuaComponent", {
      _tags = "enabled_in_hand,enabled_in_world,enabled_in_inventory",
      script_source_file = "mods/d-wonders/files/scripts/magic/decrease_draw_infinity_remove_detection.lua",
      execute_every_n_frame = 1,
    })
    AddNewInternalVariable(spell_entity_id, DECREASE_DRAW.VARIABLE_KEYS.END_WATCH_WAND, "value_int", watch_wand_component_id)
  end
end
...
local function remove_spell_out_wand_detection(spell_entity_id)
  ...
  if watch_wand_component_id ~= nil then
    EntityRemoveComponent(spell_entity_id, watch_wand_component_id)
    RemoveInternalVariableValue(spell_entity_id, DECREASE_DRAW.VARIABLE_KEYS.END_WATCH_WAND)
  end
end
...
local function decrease_actions_per_round(spell_entity_id, wand_entity_id, ability_component)
  EntityAddTag(spell_entity_id, DECREASE_DRAW.EXECUTED_MARK_TAG)

  local true_actions_per_round = GetInternalVariableValue(wand_entity_id, DECREASE_DRAW.VARIABLE_KEYS.TRUE_DRAW, "value_int")
  if true_actions_per_round == nil then
    ...
    AddNewInternalVariable(wand_entity_id, DECREASE_DRAW.VARIABLE_KEYS.TRUE_DRAW, "value_int", true_actions_per_round)
  end

  local inifinity_draw_size = GetInternalVariableValue(spell_entity_id, DECREASE_DRAW.VARIABLE_KEYS.DRAW_SIZE, "value_int")
  if inifinity_draw_size == nil then
    local current_actions_per_round = tonumber(ComponentObjectGetValue2(ability_component, "gun_config", "actions_per_round"))
    local deck_capacity = tonumber(ComponentObjectGetValue2(ability_component, "gun_config", "deck_capacity"))
    if deck_capacity < current_actions_per_round then
      inifinity_draw_size = deck_capacity - 1
    else
      inifinity_draw_size = current_actions_per_round - 1
    end

    AddNewInternalVariable(spell_entity_id, DECREASE_DRAW.VARIABLE_KEYS.DRAW_SIZE, "value_int", inifinity_draw_size)
  end

  local new_actions_per_round = true_actions_per_round - inifinity_draw_size
  SetInternalVariableValue(wand_entity_id, DECREASE_DRAW.VARIABLE_KEYS.TRUE_DRAW, "value_int", new_actions_per_round)
...
local function increase_actions_per_round(spell_entity_id, wand_entity_id, ability_component)
  EntityRemoveTag(spell_entity_id, DECREASE_DRAW.EXECUTED_MARK_TAG)

  RemoveInternalVariableValue(spell_entity_id, DECREASE_DRAW.VARIABLE_KEYS.REIGSTERED_WAND_ENTITY)

  local inifinity_draw_size = GetInternalVariableValue(spell_entity_id, DECREASE_DRAW.VARIABLE_KEYS.DRAW_SIZE, "value_int")
  RemoveInternalVariableValue(spell_entity_id, DECREASE_DRAW.VARIABLE_KEYS.DRAW_SIZE)
  if inifinity_draw_size == nil then
    print_error("Increase infinity draw don't have self draw size.")
  end
...
local function when_wand_got_decrease_draw(spell_entity_id)
  ...
  if wand_entity_id == nil then
    ...
    AddNewInternalVariable(spell_entity_id, DECREASE_DRAW.VARIABLE_KEYS.REIGSTERED_WAND_ENTITY, "value_int", wand_entity_id)
  end
  ...
  local already_decreased_actions_per_round = EntityHasTag(spell_entity_id, DECREASE_DRAW.EXECUTED_MARK_TAG)
  if already_decreased_actions_per_round then
    return
  end
...
function enabled_changed(spell_entity_id, is_enabled)
  ...
  if is_enabled then
    if EntityHasTag(parent_entity_id, "wand") then
      when_wand_got_decrease_draw(spell_entity_id)
    end
  else
