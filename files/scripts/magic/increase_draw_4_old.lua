...
local function is_wand_entity(entity_id)
  local tags = EntityGetTags(entity_id) or ''
  return string.find(tags, "wand")
end

local function increase_draw(wand_entity_id, increase_drow_size, is_enabled)
  ...
  local increase_draw_count_array_serialized = GetInternalVariableValue(wand_entity_id, 'increase_draw_4.increase_draw_count_array', 'value_string')
  if increase_draw_count_array_serialized == nil then
    increase_draw_count_array = {}
    AddNewInternalVariable(wand_entity_id, 'increase_draw_4.increase_draw_count_array', 'value_string', Json.encode(increase_draw_count_array))
  else
  ...
  if wand_true_capacity == nil then
    ...
    print('wand_true_capacity: ' .. tostring(wand_true_capacity))
  ...
  for _, ability_componet in ipairs(ability_components) do
    ...
    if is_enabled then
      local new_wand_draw = current_wand_draw + increase_drow_size
  ...
  SetInternalVariableValue(wand_entity_id, 'increase_draw_4.increase_draw_count_array', 'value_string', Json.encode(increase_draw_count_array))
end


-- TODO:リファクタリング
-- TODO:スタックがドロー別になってるので、仕組みが想定と違っている
-- 　　　（true Countがあるなら、そこから増やしたり減らしたりするだけで良さそう）
function enabled_changed(spell_entity_id, is_enabled)
  ...
  local destract_draw_status = false

  if is_enabled then
    print('enabled_changed' .. parent_entity_id)
    if is_wand_entity(parent_entity_id) then
      local wand_entity_id = GetInternalVariableValue(spell_entity_id, 'increase_draw_4.increase_draw_target_wand_entity_id', 'value_int')
      print('wand_entity_id: ' .. tostring(wand_entity_id))
      if wand_entity_id == nil then
        AddNewInternalVariable(spell_entity_id, 'increase_draw_4.increase_draw_target_wand_entity_id', 'value_int', parent_entity_id)
      end

      increase_draw(parent_entity_id, 4, is_enabled)
    end
  else
    print('disabled')
    local wand_entity_id = GetInternalVariableValue(spell_entity_id, 'increase_draw_4.increase_draw_target_wand_entity_id', 'value_int')
    destract_draw_status = wand_entity_id ~= parent_entity_id
    if wand_entity_id ~= nil then
      parent_entity_id = wand_entity_id
    end

    RemoveInternalVariableValue(spell_entity_id, 'increase_draw_4.increase_draw_target_wand_entity_id')
    if is_wand_entity(parent_entity_id) and destract_draw_status then
      increase_draw(parent_entity_id, 4, is_enabled)
    end
  end
end
