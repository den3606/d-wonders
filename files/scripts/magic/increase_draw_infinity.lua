...
local INCREASE_DRAW = dofile_once("mods/d-wonders/files/scripts/magic/increase_draw_infinity_statics.lua")
...
local function add_spell_in_wand_detection(spell_entity_id)
  ...
  if watch_wand_component_id == nil then
    watch_wand_component_id = EntityAddComponent2(spell_entity_id, "LuaComponent", {
      ...
      script_source_file = "mods/d-wonders/files/scripts/magic/increase_draw_infinity_add_detection.lua",
...
local function add_spell_out_wand_detection(spell_entity_id)
  ...
  if watch_wand_component_id == nil then
    watch_wand_component_id = EntityAddComponent2(spell_entity_id, "LuaComponent", {
      _tags = "enabled_in_hand,enabled_in_world,enabled_in_inventory",
      script_source_file = "mods/d-wonders/files/scripts/magic/increase_draw_infinity_remove_detection.lua",
      execute_every_n_frame = 1,
    })
...
local function increase_actions_per_round(spell_entity_id, wand_entity_id, ability_component)
  ...
  local inifinity_draw_size = GetInternalVariableValue(spell_entity_id, INCREASE_DRAW.VARIABLE_KEYS.DRAW_SIZE, "value_int")
  if inifinity_draw_size == nil then
    ...
    inifinity_draw_size = deck_capacity - true_actions_per_round
    AddNewInternalVariable(spell_entity_id, INCREASE_DRAW.VARIABLE_KEYS.DRAW_SIZE, "value_int", inifinity_draw_size)
  end
...
local function decrease_actions_per_round(spell_entity_id, wand_entity_id, ability_component)
  ...
  local inifinity_draw_size = GetInternalVariableValue(spell_entity_id, INCREASE_DRAW.VARIABLE_KEYS.DRAW_SIZE, "value_int")
  RemoveInternalVariableValue(spell_entity_id, INCREASE_DRAW.VARIABLE_KEYS.DRAW_SIZE)
  if inifinity_draw_size == nil then
